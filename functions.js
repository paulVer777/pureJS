...
const renderFilteredTodos = (arr, filters) => {
    ...
    const filteredArr = arr.filter((value, index) => value.title.toLowerCase().includes(filters.searchValue.toLowerCase()))
...
const removeItem = id => {
 ...
 const index = todos.findIndex((value,index)=> value.id === id)
